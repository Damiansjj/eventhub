                                ...
                                @foreach($users as $user)
                                    <tr>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex items-center">
                                                @if($user->profile_photo)
                                                    <img class="h-8 w-8 rounded-full object-cover" src="{{ asset('storage/' . $user->profile_photo) }}" alt="">
                                                @else
                                                    <div class="h-8 w-8 rounded-full bg-gray-200 flex items-center justify-center">
                                                        <span class="text-sm font-medium text-gray-500">{{ substr($user->name, 0, 1) }}</span>
                                                    </div>
                                                @endif
                                                <div class="ml-4">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                                            @if(auth()->id() !== $user->id)
                                                <form action="{{ route('admin.users.toggle-admin', ['id' => $user->id]) }}" method="POST" class="inline">
                                                    @csrf
                                                    @method('PATCH')
